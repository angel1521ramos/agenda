...
<div class="container-fluid">
  <div class="row mb-2">
    <div class="col-sm-6">
      <h1 class="m-0">Lista de doctores</h1>
    </div><!-- /.col -->
    <div class="col-sm-6">
  ...
  <div class="box">
    <div class="box-body">
      <table id="tabla-reportes" class="table table-bordered table-striped">
        ...
        <tbody>
            @foreach ($doctor as $x)<!-- la variable post debe ser creada en el post controler--->
                <tr>
                    <td>{{$x->nombre}}</td>
                    <td>{{$x->especialidad}}</td>
                    <td>
                      <form method="GET" action="{{ route('doctor.show', $x->id) }}" style="display: inline">
                        @csrf
                          <button class="btn btn-sm btn-success" ><i class="fa fa-eye"></i></button>
                      </form>
                      <form method="GET" action="{{ route('doctor.edit', $x->id) }}" style="display: inline">
                        @csrf
                        @method('UPDATE')
                        <button class="btn btn-sm btn-primary" ><i class="fa fa-pen"></i></button>
                      </form>
                      <form method="POST" action="{{ route('doctor.destroy', $x) }}" style="display: inline">
                        @csrf
                        @method('DELETE')
